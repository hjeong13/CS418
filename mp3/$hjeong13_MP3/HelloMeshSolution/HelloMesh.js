...
var eyePt = vec3.fromValues(0.0,0.0,12.0);
...
function uploadModelViewMatrixToShader_c() {
...
function uploadProjectionMatrixToShader_c() {
  gl.uniformMatrix4fv(shaderProgramCube.pMatrixUniform, 
                      false, pMatrix);
}
//-------------------------------------------------------------------------
/**
...
function uploadNormalMatrixToShader_c() {
  mat3.fromMat4(nMatrix,mvMatrix);
  mat3.transpose(nMatrix,nMatrix);
  mat3.invert(nMatrix,nMatrix);
...
function setMatrixUniforms_c() {
    uploadModelViewMatrixToShader_c();
    uploadNormalMatrixToShader_c();
    uploadProjectionMatrixToShader_c();
}
...
function setupShaders_c() {
  vertexShaderC = loadShaderFromDOM("shader-vs-cube");
  fragmentShaderC = loadShaderFromDOM("shader-fs-cube");
  ...
  shaderProgramCube.uCubeSampler = gl.getUniformLocation(shaderProgramCube, "uCubeSampler");
}
...
function drawCube(){
  ...
  // Set the texture coordinates attribute for the vertices.
    //gl.bindBuffer(gl.ARRAY_BUFFER, cubeVertexNormalBuffer);
    //gl.vertexAttribPointer(shaderProgramCube.vertexNormalAttribute, //cubeVertexNormalBuffer.itemSize, gl.FLOAT, false, 0, 0);

  // Specify the texture to map onto the faces.
    gl.bindBuffer(gl.ARRAY_BUFFER, cubeVertexPositionBuffer);
    gl.vertexAttribPointer(shaderProgramCube.vertexPositionAttribute, cubeVertexPositionBuffer.itemSize, gl.FLOAT, false, 0, 0);
    
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_CUBE_MAP, cubeMap);
    gl.uniform1i(gl.getUniformLocation(shaderProgramCube, "uCubeSampler"), 0);
    //console.log('hehe', cubeVertexPositionBuffer);
  // Draw the cube.
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, cubeTriIndexBuffer);
    setMatrixUniforms_c();
    //gl.drawArrays(gl.TRIANGLES, 0, 36); 
    gl.drawElements(gl.TRIANGLES, 36, gl.UNSIGNED_SHORT, 0);
    gl.disableVertexAttribArray(shaderProgramCube.vertexPositionAttribute);
    mvPopMatrix();
}
...
function draw() { 
    ...
    if (myMesh.loaded() == true){
        ...
        if ((document.getElementById("non_reflective").checked))
        {
